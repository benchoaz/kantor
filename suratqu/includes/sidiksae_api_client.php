...
class SidikSaeApiClient {
    ...
    public function authenticate() {
        // Auth token endpoint - base_url already includes /api/v1
     ...
     * NOTE: If API uses Key-Only, we return a dummy or null to skip Bearer auth
     * @return string|null
     */
    private function getToken() {
        // QUICK FIX: API uses X-API-KEY only. Skip JWT Auth.
        // If we force authenticate(), it hits a non-existent route or fails.
        // We return NULL here so makeRequest skips the "Authorization: Bearer" header
        return null;
    }
    ...
    public function sendDisposition($dispositionData) {
        ...
        // Use the verified working endpoint: /disposisi/push (base_url has /api/v1)
        $url = rtrim($this->config['base_url'], '/') . '/disposisi/push';
    ...
    public function updateDispositionStatus($externalId, $updateData) {
        ...
        $url = rtrim($this->config['base_url'], '/') . '/disposisi/update-status';
    ...
    public function getDispositionStatus($externalId) {
        ...
        $url = rtrim($this->config['base_url'], '/') . '/disposisi/status';
    ...
    public function getSuratDetail($id_surat) {
        ...
        $url = rtrim($this->config['base_url'], '/') . '/surat/' . $id_surat;
    ...
    public function testConnection() {
        // Test with actual API endpoint that exists: GET /api/surat
        // base_url already includes /api, so we just add /surat
        $url = rtrim($this->config['base_url'], '/') . '/surat';
        
        $response = $this->makeRequest('GET', $url, null, false);
        
        // If we get any valid response (200 or 401 means API is working)
        // 401 is OK for test - it means API is there but needs auth
        // 200 means API works and we can access
        if ($this->lastHttpCode == 200) {
            return [
                'success' => true,
                'message' => '✓ Koneksi berhasil! API dapat diakses dengan baik.',
                'http_code' => 200
            ];
        } elseif ($this->lastHttpCode == 401) {
            return [
                'success' => true,
                'message' => '✓ API terhubung (perlu autentikasi untuk akses data).',
                'http_code' => 401
            ];
        } elseif ($this->lastHttpCode == 404) {
            return [
                'success' => false,
                'message' => '✗ Endpoint tidak ditemukan. Periksa konfigurasi URL API.',
                'http_code' => 404
            ];
        }
        ...
        return [
            'success' => false,
            'message' => '✗ Tidak dapat terhubung ke API SidikSae. HTTP ' . $this->lastHttpCode,
            'http_code' => $this->lastHttpCode
        ];
    }
    ...
    private function makeRequest($method, $url, $data = null, $authenticated = false, $isMultipart = false) {
        ...
        $headers = [
            ...
            'X-APP-ID: ' . ($this->config['app_id'] ?? $this->config['client_id'] ?? 'suratqu'),
    ...
    /**
     * Register Surat (Metadata + File) to API
     * @param array $data payload data
     * @return array Response
     */
    public function registerSurat($data) {
        $url = rtrim($this->config['base_url'], '/') . '/surat';
        
        // Ensure data is sent as JSON with authentication
        ...
        if ($response && (
            ...
            return [
                'success' => true,
                'data' => $response,
                'message' => 'Surat registered successfully',
                'http_code' => $this->lastHttpCode
            ];
        }
        ...
        return [
            'success' => false,
            'error' => $response['message'] ?? $response['error'] ?? 'Failed to register surat',
            'http_code' => $this->lastHttpCode,
            'response' => $response
        ];
    }
