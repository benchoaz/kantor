...
try {
    ...
    // Using uuid_user column (must be added first via migration)
    $stmt = $apiDb->query("
        SELECT uuid_user, username, password, nama as full_name,
               CASE WHEN is_active = 1 THEN 'active' ELSE 'inactive' END as status
        FROM users
        WHERE username IS NOT NULL AND username != ''
        AND uuid_user IS NOT NULL AND uuid_user != ''
    ");
