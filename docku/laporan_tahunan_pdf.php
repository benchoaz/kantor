<?php
// laporan_tahunan_pdf.php - Generate Annual Report PDF
error_reporting(E_ALL);
ini_set('display_errors', 0); // Hide errors to prevent PDF corruption
...
$stats_by_month = array_fill(1, 12, 0);
...
foreach ($activities as $k) {
    ...
    // Count Types (Fix for undefined variable)
    $type = $k['tipe_kegiatan'] ?? 'biasa';
    if (isset($stats_by_type[$type])) {
        $stats_by_type[$type]++;
    } else {
        $stats_by_type['biasa']++; // Default fallback
    }

    $bulan = (int)date('n', strtotime($k['tanggal']));
    $stats_by_month[$bulan]++;
}
...
class PDF extends PDF_Base {
    function Footer() {
        ...
        $this->Cell(0, 10, 'Laporan Tahunan / ' . date('d/m/Y H:i') . ' - Halaman ' . $this->PageNo() . '/{nb}', 0, 0, 'C');
    }
}
...
$pdf->Cell(0, 10, 'LAPORAN KEGIATAN TAHUNAN', 0, 1, 'C');
...
$pdf->Cell(0, 8, 'I. REKAPITULASI TAHUNAN', 0, 1, 'L');
$pdf->Ln(2);
...
$pdf->SetFont('Arial', 'B', 11);
$pdf->SetX($pdf->GetLMargin() + 5);
$pdf->Cell(0, 7, 'Trend Kegiatan Per Bulan:', 0, 1);
$pdf->SetFont('Arial', '', 10);

for ($m = 1; $m <= 12; $m++) {
    if ($m % 2 == 1) {
        $pdf->SetX($pdf->GetLMargin() + 10);
        $pdf->Cell(60, 5, $months[$m], 0, 0);
        $pdf->Cell(5, 5, ':', 0, 0);
        $pdf->Cell(20, 5, $stats_by_month[$m] . ' kegiatan', 0, 0);
    } else {
        $pdf->Cell(10, 5, '', 0, 0); // spacing
        $pdf->Cell(60, 5, $months[$m], 0, 0);
        $pdf->Cell(5, 5, ':', 0, 0);
        $pdf->Cell(0, 5, $stats_by_month[$m] . ' kegiatan', 0, 1);
    }
}
if (12 % 2 == 1) $pdf->Ln(5); // Close last odd row

$pdf->Ln(8);

// Activity List (Table Format)
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 8, 'II. DAFTAR KEGIATAN TAHUNAN', 0, 1, 'L');
$pdf->Ln(2);
...
$pdf->AddPage($orient);
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 8, 'III. NARASI EVALUASI TAHUNAN', 0, 1, 'L');
$pdf->Ln(2);

$pdf->SetFont('Arial', '', 11);
$pdf->SetX($pdf->GetLMargin() + 10);

$avg_per_month = round($total_kegiatan / 12, 1);
$max_month = array_search(max($stats_by_month), $stats_by_month);
$max_month_name = $months[$max_month];

$narasi = "Sepanjang tahun {$tahun}, {$bidang_name} telah melaksanakan dan mendokumentasikan {$total_kegiatan} kegiatan dengan rata-rata {$avg_per_month} kegiatan per bulan. ";
...
$narasi .= "Puncak aktivitas terjadi pada bulan {$max_month_name} dengan {$stats_by_month[$max_month]} kegiatan terdokumentasi. ";
$narasi .= "Seluruh kegiatan telah didokumentasikan dengan bukti foto dan laporan terstruktur sesuai standar tata kelola pemerintahan, siap sebagai bahan evaluasi kinerja dan pelaporan resmi.";
